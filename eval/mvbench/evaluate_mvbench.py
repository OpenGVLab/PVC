...
def collate_fn(batches, tokenizer):
    ...
    split_sizes = torch.cat([_['split_sizes'] for _ in batches])
    data_flag = torch.cat([_['data_flag'] for _ in batches])
    questions = [_['question'] for _ in batches]
    answers = [_['answer'] for _ in batches]
    num_patches_lists = [_['num_patches_list'] for _ in batches]
    task_types = [_['task_type'] for _ in batches]
    return pixel_values, split_sizes, data_flag, questions, answers, num_patches_lists, task_types
...
class MVBenchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'split_sizes': torch.LongTensor((pixel_values.shape[0], )),
            'data_flag': torch.LongTensor((3, )),
            'answer': answer,
...
def evaluate_chat_model():
    ...
    outputs = []
    for _, (pixel_values, split_sizes, data_flag, questions, answers, num_patches_lists, task_types) in tqdm(enumerate(dataloader)):
